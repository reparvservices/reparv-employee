...
const CallEnquirers = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!response.ok) throw new Error("Failed to fetch call enquiries.");
  ...
  const del = async (id) => {
    ...
    try {
      const response = await fetch(URI + `/admin/call-enquirers/delete/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
  ...
  const status = async (id) => {
    ...
    try {
      const response = await fetch(URI + `/admin/call-enquirers/status/${id}`, {
        method: "PUT",
        credentials: "include",
      });
      const data = await response.json();
  ...
  const columns = [
    ...
    {
      name: "Type",
      selector: (row) => <span className={``}>{row.type}</span>,
      sortable: true,
      minWidth: "150px",
    },
    {
      name: "Category",
      selector: (row) => <span className={``}>{row.category}</span>,
      sortable: true,
      minWidth: "150px",
    },
    {
  ...
  return (
    ...
    >
      <div className="role-table w-full h-[80vh] flex flex-col px-4 md:px-6 py-6 gap-4 my-[10px] bg-white rounded-[24px]">
        ...
        <h2 className="text-[16px] font-semibold">Whatsapp Enquiry List</h2>
