...
const Subscribers = () => {
  ...
  // Delete record
  const del = async (id) => {
    if (!window.confirm("Are you sure to delete this Subscriber ?")) return;
    try {
      const response = await fetch(URI + `/admin/subscribers/delete/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
  ...
  const status = async (id) => {
    ...
    try {
      const response = await fetch(URI + `/admin/subscribers/status/${id}`, {
        method: "PUT",
        credentials: "include",
      });
      const data = await response.json();
  ...
  return (
    ...
    >
      <div className="role-table w-full h-[80vh] flex flex-col px-4 md:px-6 py-6 gap-4 my-[10px] bg-white rounded-[24px]">
        ...
        <h2 className="text-[16px] font-semibold">Subscribers List</h2>
